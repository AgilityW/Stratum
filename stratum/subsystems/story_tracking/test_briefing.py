...
from stratum.subsystems.story_tracking import generate_context, format_context_for_prompt
from stratum.subsystems.story_tracking.briefing_context import (
    _active_chains,
    _carried_forward,
    _coverage_gaps,
    _due_judgments,
...
from stratum.subsystems.story_tracking import ContextSelectionPolicy
...
class TestCarriedForward:
    def test_context_selection_policy_owns_carried_forward_ranking(self):
        events = [
            make_event(1, priority=4, scale_refs=[
        ...
        result = ContextSelectionPolicy().carried_forward(events, "daily", "2026-06-02", 7)
...
class TestDueJudgments:
    ...
    def test_includes_deferred_judgments_when_verification_window_is_due(self):
        judgments = [
            make_judgment(1, verdict="deferred", expected_verification="2026-06-07"),
        ]
        ...
        assert result[0]["due_basis"] == "expected_verification"

    def test_uses_shared_judgment_lifecycle_fallback_for_free_text_windows(self):
        judgments = [
            make_judgment(
                1,
                made_at="2026-06-01",
        ...
        assert result[0]["due_basis"] == "created_at"

    def test_excludes_expected_verification_after_due_window(self):
        judgments = [
            make_judgment(
                1,
                made_at="2026-06-01",
                expected_verification="2026-06-20",
            ),
        ]
