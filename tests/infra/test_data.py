...
These tests inspect the latest local run under:
    {config.reports_dir or config.output_dir}/{domain}/data/{YYYY-MM-DD}/
...
def _output_root() -> Path:
    ...
    for _ in range(5):
        ...
        if os.path.exists(cfg_path):
            ...
            root = cfg.get("reports_dir") or cfg.get("output_dir", "~/stratum/reports")
            return Path(os.path.expanduser(root))
        current = os.path.dirname(current)
    return Path(os.path.expanduser("~/stratum/reports"))
...
def _write_golden_run_dir(run_dir: Path) -> Path:
    ...
    artifact_base = "Storage_Daily_Briefing_2026-05-30"
    ...
    raw_results = [
        {
            ...
            "query_id": "watchlist-sk-hynix-newsroom",
            "query_used": "watchlist source",
    ...
    _write_json(run_dir / "watchlist_stats.json", {
        "domain": "storage",
        "date": "2026-05-30",
        "total_results": 1,
        "sources": [
    ...
    (run_dir / f"{artifact_base}.html").write_text(
        "<!doctype html><html><body><h1>Storage Daily Briefing</h1></body></html>"
    )
    (run_dir / f"{artifact_base}.pdf").write_bytes(b"%PDF-1.4\n% fixture pdf\n")
...
class TestJsonParseable:
    ...
    def test_raw_stats_json_is_valid_if_present(self, current_run_dir):
        ...
        schema = _contract_schema("search_stats.json")
        data = json.loads(path.read_text())
    ...
    def test_raw_json_matches_contract_if_present(self, current_run_dir):
        ...
        schema = _contract_schema("search_result.json")
        data = json.loads(path.read_text())
    ...
    def test_watchlist_stats_json_is_valid_if_present(self, current_run_dir):
        run_dir = current_run_dir
        path = run_dir / "watchlist_stats.json"
        if not path.exists():
            pytest.skip("No watchlist_stats.json")
        schema = _contract_schema("watchlist_stats.json")
        data = json.loads(path.read_text())
        validate(data, schema)
...
class TestWatchlistStatsConsistency:
    def test_legacy_collector_stats_sidecar_is_removed(self, current_run_dir):
        assert not (current_run_dir / "collector_stats.json").exists()

    def test_watchlist_stats_have_current_fields(self, current_run_dir):
        run_dir = current_run_dir
        path = run_dir / "watchlist_stats.json"
        if not path.exists():
            pytest.skip("No watchlist_stats.json")
        ...
        for i, source in enumerate(data.get("sources", []), 1):
            ...
            assert not missing, f"watchlist source {i}: missing {sorted(missing)}"
