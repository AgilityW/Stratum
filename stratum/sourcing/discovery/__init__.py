...
def run_search(
    ...
    from stratum.sourcing.discovery.config import load_api_keys, load_search_config
    ...
    from stratum.sourcing.discovery.models import Query, ResultSet
    ...
    query_planning = config.get("query_planning", {}) or {}
    queries, retired_queries, query_selection_decisions = QueryPlanner().apply_performance(
        queries,
        config.get("query_performance"),
        retire_low_yield=bool(query_planning.get("retire_low_yield")),
    )
    ...
    raw_results, query_stats = execute(
        ...
        end_date=end_date,
        engine_health=config.get("engine_health"),
    )

    total_raw = len(raw_results)
    observations = observations_from_results(raw_results, query_objects)
    ...
    diagnostics["engine_configuration"] = _engine_configuration_diagnostics(
        config=config,
        engines=engines,
    )
    diagnostics["query_selection"] = {
        "retired_queries": [query.get("id", "") for query in retired_queries],
        "decisions": [decision.__dict__ for decision in query_selection_decisions],
    }
...
def build_diagnostics(
    ...
    diagnostics = {
        ...
        "query_performance": QueryPerformanceScorer().diagnostics(query_stats),
        "engine_health": score_search_engine_health([s.to_dict() for s in query_stats]),
    }
    return diagnostics


def _engine_configuration_diagnostics(
    config: dict[str, Any],
    engines: dict[str, Any],
) -> dict[str, Any]:
    """Describe configured vs usable discovery engines for operator diagnostics."""
    configured_engines = list((config.get("engines") or {}).keys())
    usable_engines = list(engines.keys())
    missing_api_keys = sorted(
        engine_name for engine_name in configured_engines if engine_name not in engines
    )
    unreachable_locales = []
    for locale, fallback_order in sorted(
        (config.get("routing") or {}).items(),
        key=lambda item: str(item[0]),
    ):
        if any(engine_name in engines for engine_name in fallback_order):
            continue
        unreachable_locales.append({
            "locale": _diagnostic_key(locale),
            "configured_order": list(fallback_order),
        })
    return {
        "configured_engines": configured_engines,
        "usable_engines": usable_engines,
        "missing_api_keys": missing_api_keys,
        "unreachable_locales": unreachable_locales,
    }
...
__all__ = [
    "QueryPlanner",
    "QueryPerformanceScorer",
    "Query",
    "QueryStats",
    "ResultSet",
    "SearchDiversityRanker",
    "SearchResult",
    "SearchResultScorer",
...
def __getattr__(name: str):
    ...
    if name in (
        ...
        from stratum.sourcing.discovery.query_planner import (
            QueryPerformanceScorer,
            QueryPlanner,
            SearchSupplementPolicy,
            split_queries_by_coverage,
        ...
        return locals()[name]
    if name in ("SearchDiversityRanker", "SearchResultScorer"):
    ...
    if name in (
        ...
        from stratum.sourcing.discovery.models import (
            Query,
            QueryStats,
            ResultSet,
            SearchResult,
        ...
        return locals()[name]
    raise AttributeError(f"module 'stratum.sourcing.discovery' has no attribute '{name}'")
