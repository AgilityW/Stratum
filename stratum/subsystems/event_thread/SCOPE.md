# event_thread - event lifecycle and cross-temporal linking

## Purpose

`stratum/subsystems/event_thread` provides deterministic event-thread mechanics: lifecycle status, cluster-to-thread matching, watch query generation, archive rules, and cross-scale linking.

It is a computational subsystem. Long-lived persistence currently belongs to `stratum/db`.

The package entrypoint `stratum.subsystems.event_thread` is the stable import
surface for lifecycle, matching, watch-query, and cross-temporal helpers.

## Modules

| File | Role |
|:---|:---|
| `event_thread.py` | daily thread orchestration, matching, updates, watch-query handoff |
| `lifecycle_policy.py` | reusable lifecycle scoring policy for event-thread status, momentum, and archive readiness |
| `cross_temporal.py` | daily/weekly/monthly/quarterly/yearly appearance registration and rollup tracing |
| `__init__.py` | stable package surface for event-thread helpers |
...
- Score lifecycle: `emerging -> active -> cooling -> resolved -> archived`,
  including momentum, archive readiness, and watch-query ranking score.
...
- Lifecycle status is computed from timeline entries visible at the target
  ...
  update.
- Lifecycle scoring is owned by `ThreadLifecycleScorer` in
  `stratum.subsystems.event_thread.lifecycle_policy`. The event-thread engine
  calls the scorer when computing status, archive
  readiness, and watch-query ordering; it does not embed those thresholds in
  orchestration code.
- The scorer returns structured status, momentum, score, days-since-last,
  archive readiness, and reason fields. Higher-scale synthesis ranking consumes
  the score to promote active/escalating stories and demote cooling or resolved
  stories without reimplementing lifecycle rules.
- `evaluate_thread_lifecycle()` and `lifecycle_diagnostics()` expose those
  fields as JSON-compatible diagnostics. `evolve_threads()` includes the
  diagnostics in its result so review and later persistence can consume
  lifecycle decisions without reparsing status strings.
...
- Watch query generation prioritizes high-priority threads before medium/low
  threads when the daily query cap is reached. Within the same priority,
  lifecycle score wins before recency, so active or escalating stories are not
  displaced by lower-momentum cooling stories simply because the cooling record
  has a later administrative update.
...
- The daily orchestrator persists generated watch queries into SQLite as
  ...
  the next acquisition run without making this subsystem own persistence.
...
- `test_thread.py` covers lifecycle scoring, matching, creation/update,
  watch query generation, and evolution.
- `test_temporal.py` covers appearance registration, rollup, trace, resolution, and summaries.
