"""Browser watchlist — headless Chrome via Playwright.
...
from stratum.sourcing.watchlist.direct_fetch import (
    _ArticleExtractor,
    _extract_date,
...
def fetch_source(
    ...
    try:
        ...
        for url in urls:
            try:
                ...
                if heading_links:
                    heading_results = _convert_links(
    ...
    if keywords is not None:
        admitted, candidates = admit_results_with_candidates(
            results,
            keywords,
            source_id=source_id,
            access="browser",
...
def collect(domain: str, workspace: str, run_date: str) -> list[SearchResult]:
    """Search configured sources from all active browser sources.
