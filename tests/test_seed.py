...
def test_update_query_stats_persists_search_engine_health(monkeypatch, tmp_path):
    from stratum.db.ingest import update_query_stats
    ...
    """)
    conn.execute("""
        INSERT INTO queries (id, text, locale, intent)
        VALUES ('q-1', 'Samsung HBM4', 'en', 'detection')
    """)
    conn.commit()
    conn.close()
    ...
    update_query_stats(
        "storage",
        [{"query_id": "q-1", "results_count": 0, "status": "failed"}],
        run_date="2026-05-30",
        engine_health={
            "tavily": {
                "engine": "tavily",
    ...
    check.row_factory = sqlite3.Row
    row = check.execute(
        "SELECT attempts, failure_rate, recommendation, errors FROM search_engine_health WHERE engine = 'tavily'"
    ).fetchone()
    check.close()
    ...
    assert json.loads(row["errors"]) == ["quota"]
